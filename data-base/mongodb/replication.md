...
**心跳机制（Hearbeat）：** 复制集成员间默认**每两秒**会发送一次心跳信息，如果**十秒**未收到某个节点的心跳，则认为该节点已宕机不可以访问；如果宕机的节点为 Primary，Secondary（前提是可被选为 Primary）会发起新的 Primary 选举。仲裁员与其他集合成员之间的唯一沟通是：选举期间的投票，心跳和配置数据，而且这些交换未加密。
...
如果复制集中的主节点出现故障，那么主节点无法继续处理客户发出的请求。在 MongoDB 复制集中只有主节点可以处理客户发出的 **写** 请求，一旦主节点发生故障，客户难道无法继续往复制集中写任何数据吗？这种情况显然是无法满足高可用性的要求的。

复制集当中的主节点并非一成不变，同时也非使用者指定的而一直不会变化的，复制集当中的主节点其实是由整个复制集通过选举算法推选出来的。那么当原本的主节点无法继续工作的时候，会通过一次新的选举来从复制集当中其他的节点重选出新的主节点，如此便能恢复复制集的正常运作，主节点也能继续处理客户的写操作。

下面通过示例图描述复制集选举的过程：

![](../../snapshots/mongodb/mongodb-selection.jpg)

在讲解复制集选举过程前，需要提及的是：复制集当中每个节点都会有所谓的选举计数器，英文称为 `term`。计数器在每次在节点参与新的选举时候，都会将自身的选举计数器自增一，默认为 `N`，表示所有节点到目前为止都参与了 `N` 次选举。

假设主节点 A 出现故障（崩溃、下线），其余两个副节点均可发起复制集选举。由于主节点 A 本应不停地为其余副节点发送心跳请求，当主节点 A 出现故障，那么很可能本应该发送的心跳请求超时，使得其他副节点发现主节点不再发送心跳请求，很可能原有的主节点不再工作。在这种情况下，任意副节点都可以发起一场新的选举，当然在实际情况当中，我们可以为不同的节点设置不同的优先级别，当各个节点的优先级别不同时，MongoDB 的算法会尽最大的努力，让优先级别高的副节点成为最先发起选举的节点，也就是说优先级别高的副节点有更大的可能性成为新的主节点。
...
候选节点会把投票请求发送给复制集中的所有节点，当然包括失联的主节点。原本的主节点可能存在不正常的状态，他未必能响应成功的结果，那么会视该节点投了反对票

当候选节点收集了复制集中其他节点的投票结果之后，如果超过半数的其他节点（包括该候选节点本身所投的票）给该候选节点投了赞成票，那么该候选节点会当选成新的主节点。这时候该复制集将会恢复正常的运作，可以继续处理客户发来的读写操作
...
- 主节点与副节点之间的心跳请求超时
- 复制集初始化：初始化时还没有主节点和副节点
- 新节点加入复制集：这里说的新节点未必是全新的节点，也可能是原来失联的节点重新连接回复制集当中
...
**根据部署结构部署复制集示例** **三个节点成员的副本集，成员合理分布以及解析如下**：
